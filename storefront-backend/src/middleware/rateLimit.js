...
function createRateLimiter({ name, limit, windowMs, key = (req) => clientIp(req), message = "Too many requests. Please try again later.", failClosed = false, code = "RATE_LIMITED" }) {
  return async function rateLimit(req, res, next) {
    ...
    try {
      ...
      if (Number(doc?.count || 0) > limit) {
        ...
        return next(new ApiError(429, message, { code, details: { retryAfterSeconds: retryAfter } }));
      }
      return next();
    } catch (err) {
...
// Burst guard on /request-otp: on top of the per-request cooldown enforced
// in the controller, this caps how many send attempts one client+email pair
...
// Separate bucket for /verify-otp — deliberately NOT shared with
// otpRateLimit above. Verify attempts (right or wrong) must never eat into
// the budget for *sending* a code, or a few wrong guesses would lock out
// the resend button before the OTP itself even expires.
const otpVerifyRateLimit = createRateLimiter({ failClosed: true, name: "otp-verify", limit: env.otpRateLimit, windowMs: env.otpRateWindowMs, key: (req) => `${clientIp(req)}:${String(req.body?.email || "").trim().toLowerCase()}` });
// Hard daily cap on OTP *sends* per email: at most OTP_DAILY_LIMIT codes
// (default 5) can be requested for a given address in a rolling day,
// independent of the 15-minute burst window above and the 60s cooldown
// enforced in the controller. Keyed on email only (not IP) — it's an
// account-level quota, not a per-client one.
const otpDailyRateLimit = createRateLimiter({
  failClosed: true,
  name: "otp-daily",
  limit: env.otpDailyLimit,
  windowMs: 24 * 60 * 60 * 1000,
  key: (req) => String(req.body?.email || "").trim().toLowerCase(),
  message: "You've reached today's limit for verification codes. Please try again tomorrow.",
...
module.exports = { createRateLimiter, generalRateLimit, authRateLimit, otpRateLimit, otpVerifyRateLimit, otpDailyRateLimit, paymentRateLimit, sensitiveRateLimit };
