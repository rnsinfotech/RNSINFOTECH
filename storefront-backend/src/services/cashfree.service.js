...
const WEBHOOK_MAX_AGE_MILLISECONDS = 5 * 60 * 1000;
const CASHFREE_WEBHOOK_VERSION = "2025-01-01";
...
async function createCashfreeOrder({ orderId, amountInRupees, currency = "INR", customer, returnUrl, notifyUrl, expiresAt, note }) {
  ...
  return cashfreeRequest("/orders", {
    ...
    idempotencyKey: deterministicUuid(`cashfree-order:${orderId}`),
  });
}
 ...
 */
function verifyWebhookSignature({ rawBody, signature, timestamp, webhookVersion, maxAgeMilliseconds = WEBHOOK_MAX_AGE_MILLISECONDS }) {
  if (!env.cashfree.secretKey) return false;
  if (typeof signature !== "string" || !signature) return false;
  if (typeof timestamp !== "string" || !timestamp) return false;
  if (typeof webhookVersion !== "string" || webhookVersion !== CASHFREE_WEBHOOK_VERSION) return false;

  // Cashfree's current webhook timestamp is Unix epoch milliseconds.
  const timestampMs = Number(timestamp);
  const ageMilliseconds = Math.abs(Date.now() - timestampMs);
  if (!Number.isSafeInteger(timestampMs) || !Number.isFinite(ageMilliseconds) || ageMilliseconds > maxAgeMilliseconds) return false;
...
module.exports = {
  ...
  REFUND_STATUS,
  WEBHOOK_TYPES,
  WEBHOOK_MAX_AGE_MILLISECONDS: WEBHOOK_MAX_AGE_MILLISECONDS,
};
